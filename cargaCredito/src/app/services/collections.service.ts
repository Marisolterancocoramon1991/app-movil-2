...
export class CollectionsService {
 ...
 async delete(collectionName: string, id: string) {
  ...
  async deleteByEmail(collectionName: string, email: string) {
    const q = query(collection(this._firestore, collectionName), where("email", "==", email));
  
    const querySnapshot = await getDocs(q);
    
    // Iterar por los documentos que coinciden con el email
    querySnapshot.forEach(async (document) => {
      await deleteDoc(doc(this._firestore, collectionName, document.id));
      console.log(`Documento con email ${email} eliminado.`);
    });
